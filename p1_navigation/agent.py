...
class Agent(object):
    ...
    def __init__(self, args, state_size, action_size):
        ...
        self.qnetwork_local = DQN(args, self.state_size, self.action_size).to(device=args.device)
        if args.model and os.path.isfile(args.model):
            # Always load tensors onto CPU by default, will shift to GPU if necessary
            self.qnetwork_local.load_state_dict(torch.load(args.model, map_location='cpu'))
        ...
        self.optimizer = optim.Adam(self.qnetwork_local.parameters(), lr=args.lr, eps=args.adam_eps)
    ...
    def act(self, state):
        ...
        """
        with torch.no_grad():
            return (self.qnetwork_local(state.unsqueeze(0).to(self.device)) * self.support).sum(2).argmax(1).item()
    ...
    def learn(self, mem):
        ...
        log_ps = self.qnetwork_local(states, log=True)  # Log probabilities log p(s_t, *; theta online)
        log_ps_a = log_ps[range(self.batch_size), actions]  # log p(s_t, a_t; theat online)

        with torch.no_grad():
            # Calculate nth next state probabilities
            pns = self.qnetwork_local(next_states)  # Probabilities p(s_t+n, *; theta online)
    ...
    def soft_update(self, tau=1e-3):
        ...
        Args:
           param1 (float): tau
        """
        for target_param, local_param in zip(self.target_net.parameters(), self.qnetwork_local.parameters()):
    ...
    def evaluate_q(self, state):
        ...
        """
        with torch.no_grad():
            return (self.qnetwork_local(state.unsqueeze(0)) * self.support).sum(2).max(1)[0].item()
